		...
		EditorData *data)
{
	...
	if (g_signal_handler_is_connected (widget, data->signal_handles[HANDLER_ID_EDITOR_DESTROY]))
		g_signal_handler_disconnect (widget, data->signal_handles[HANDLER_ID_EDITOR_DESTROY]);
}
										...
										gpointer user_data)
{
	...
	{
		word = gtk_snippets_editor_get_current_word(data->editor);
		...
		gtk_snippets_editor_get_cursor_position(data->editor,&x,&y);
